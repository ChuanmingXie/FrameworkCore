...
namespace FrameworkCore.Api.Controllers
{
    ...
    public class FileUploadController : ControllerBase
    {
        ...
        public FileUploadController(ILogger<FileUploadController> logger, IWebHostEnvironment hostingEnvironment)
        {
            this.logger = logger;
        ...
        public async Task<IActionResult> UploadFiles()
        {
            try
            {
                ...
                if (Request != null && Request.Form != null &&
                    Request.Form.Files != null && Request.Form.Count > 0)
                ...
                List<string> filePathRequestList = new List<string>();
                foreach (var formFile in formFiles)
                {
                    if (formFile.Length > 0)
                    {
                        ...
                        filePathRequestList.Add($"/Files/{dateTime.ToString("yyyyMMdd")}/{fileName}");
                    }
                }
                ...
                return Ok(new
                {
                    Success = true,
                    Message = message,
                    FilePathList = filePathRequestList
                });
            }
            catch (Exception ex)
            {
